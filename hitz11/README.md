# Hitz11

Hitz11 is a small daily etymology publisher for Telegram channels.

- Source crawl: DEEILE pages from josecanovas.com
- Storage: SQLite
- Selection: random word of the day
- Weekend mode: recap from already posted words
- Scheduler: GitHub Actions at 11:11 Europe/Madrid

## 1) Requirements

- Python 3.11+
- uv

## 2) Local setup

```bash
uv sync --extra dev
cp .env.example .env
```

Fill `.env` with your real Telegram values.

## 3) Telegram setup (first time)

1. Open Telegram and talk to `@BotFather`.
2. Run `/newbot` and create your bot.
3. Copy the bot token into `TELEGRAM_BOT_TOKEN`.
4. Create a channel (public or private).
5. Add the bot as channel admin (allow posting messages).
6. Set `TELEGRAM_CHAT_ID`:
   - public channel: `@your_channel_name`
   - private channel: numeric chat id

## 4) Run locally

Crawl and update DB:

```bash
uv run hitz11 --db-path hitz11.db crawl
```

Preview one random post without sending:

```bash
uv run hitz11 --db-path hitz11.db post --dry-run --skip-time-check
```

Run full daily flow (crawl + post) without sending:

```bash
uv run hitz11 --db-path hitz11.db daily --dry-run --skip-time-check
```

## 5) Testing

Run tests:

```bash
uv run pytest
```

Run lint:

```bash
uv run ruff check .
```

## 6) GitHub Actions deployment

Workflow file: `.github/workflows/hitz11.yml`

Set repository secrets:

- `TELEGRAM_BOT_TOKEN`
- `TELEGRAM_CHAT_ID`

Optional repository variables:

- `TIMEZONE` (default `Europe/Madrid`)
- `ORIGIN_KEY` (default `basque`)
- `RECENCY_DAYS` (default `30`)

The workflow stores state in a GitHub Release called `hitz11-state`.
It downloads `hitz11.db`, runs the daily command, then uploads the updated DB back as a release asset.

## 7) Behavior rules

- Monday-Friday: random unposted entry.
- Saturday-Sunday: random recap from posted entries.
- No more than one post per local day.
- Recency window avoids short-term repeats.
